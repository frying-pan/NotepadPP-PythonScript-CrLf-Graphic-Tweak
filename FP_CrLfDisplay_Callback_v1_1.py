...
# *options values* : set value to an INTEGER >= -1
...
class C_CrLfDisplay_CB():
	# class constructor
	def __init__(self, s_script_name, i_none_width, i_norm_width, i_feature_on, \
			s_editorprop_cb_on, s_editorprop_width_cr, s_editorprop_width_lf):
		self.CBDone = False
	...
	def InputCrLfWidth(self, s_cur_width_cr, s_cur_width_lf):
		while True:
			s_response = notepad.prompt( \
				"Enter new CR and LF integer widths (>= " + str(self.none_width) + "), separated by a space, eg. : 2 1" + "\n" + \
				"Special widths : " + \
			...
			if s_response is None:
				return None
			if s_response == "":
				return (self.norm_width, self.norm_width)
...
o_crlfdisplay_cb = C_CrLfDisplay_CB(s_script_name, i_none_width, i_norm_width, i_true, \
						s_editorprop_cb_on, s_editorprop_width_cr, s_editorprop_width_lf)
