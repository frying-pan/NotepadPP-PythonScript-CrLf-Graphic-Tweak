...
t_start_width = (i_start_width_cr, i_start_width_lf)
...
class C_CrLfDisplay_CB():
	...
	def __init__(self, s_script_name, i_none_width, i_norm_width, i_start_width_cr, i_start_width_lf, \
			i_option_on, s_editorprop_cb_on, s_editorprop_width_cr, s_editorprop_width_lf):
		self.cb_done = False
	...
	def RegCallback(self):
		...
		def CB_CrLfDisplay_BufferActivated(args):
		...
		notepad.callback(CB_CrLfDisplay_BufferActivated, [NOTIFICATION.BUFFERACTIVATED])
		self.cb_done = True
...
def Main():
	...
	(i_start_width_cr, i_start_width_lf) = t_start_width
	...
	if console.editor.getProperty(s_editorprop_cb_reg) != str(i_true):
		...
		if (i_start_width_cr == i_norm_width and i_start_width_lf == i_norm_width):
			...
			print "\t" + s_callback_name + " registered but not activated (default Notepad++ display), " + \
	...
	if (i_width_cr != i_norm_width or i_width_lf != i_norm_width):
		console.editor.setProperty(s_editorprop_cb_on, str(i_true))
		print "\t" + s_callback_name + " re-activated, " + \
